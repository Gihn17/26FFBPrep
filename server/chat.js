...
- All player value/VBD/tier/auction-$ numbers come from the tools below, which run the same verified pipeline the live draft board uses — never estimate a number yourself when a tool can give the real one. Each player's value carries a valueSource field: "udk_import" means it's a real number from Will's own UDK auction-value import (the more authoritative source when present), "computed" means it's this pipeline's own VBD-proportional split of the $200x12 budget. Mention which kind you're citing when it's relevant to how much to trust the number.
