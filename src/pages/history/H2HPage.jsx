...
function TeamBlock({ team, record, align, logo, league }) {
  const name = team?.owner_name || team?.team_name || "—";
  const hot = record?.streakType === "W" && record.streakLen >= HOT_STREAK_MIN;
  return (
    <Link to={team?.owner_guid ? `/league/${league}/teams/${ownerSlug(team.owner_guid)}` : "#"}
...
function MatchupCard({ g, currentLogos, league }) {
  ...
  return (
    <div style={{ ...panelStyle(), marginBottom:10 }}>
      <div style={{ display:"flex", alignItems:"center", gap:16, flexWrap:"wrap" }}>
        <TeamBlock team={g.homeTeam} record={g.homeRecord} align="left" logo={currentLogos.get(g.homeTeam?.owner_guid)} league={league} />
        ...
        <TeamBlock team={g.awayTeam} record={g.awayRecord} align="right" logo={currentLogos.get(g.awayTeam?.owner_guid)} league={league} />
...
function WeekView({ teamIdx, matchups, teams, seasons, currentLogos, league }) {
  ...
  return (
    <>
      ...
      {slate.length === 0 ? (
        ...
        slate.map((g, i) => <MatchupCard key={i} g={g} currentLogos={currentLogos} league={league} />)
      )}
    </>
  );
}
...
export default function H2HPage() {
  const { league, teams, matchups, teamIdx, ownerOptions, seasons, currentLogos } = useOutletContext();
  ...
  return (
    <>
      ...
      {mode === "week" ? (
        <WeekView teamIdx={teamIdx} matchups={matchups} teams={teams} seasons={seasons} currentLogos={currentLogos} league={league} />
      ) : (
